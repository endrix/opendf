...
public class Cal2C extends PhasedSimulator {
	...
	public Cal2C(ConfigGroup configuration, String[] args) {
		...
		List<String> unparsed = ConfigCLIParseFactory.parseCLI(args,
				configuration);

		if (((ConfigBoolean) configuration.get(ConfigGroup.VERSION)).getValue()
				.booleanValue()) {
			configuration.usage(Logging.user(), Level.INFO);
			return;
		}

		ConfigString topName = (ConfigString) configuration
				.get(ConfigGroup.TOP_MODEL_NAME);
